/**
 * lib/id/unified-id.ts — Immutable Unified Identifier
 *
 * Replaces 36-char UUIDs with 13-char case-insensitive HEX32 strings.
 * Based on FNV-1a x64 hash of random UUID bytes.
 *
 * Properties:
 *   - 13 characters (vs 36 for UUID) — saves ~64% tokens in LLM context
 *   - Case-insensitive (symbols 0-9, A-V)
 *   - Uniform distribution for partitioning
 *   - Deterministic from input bytes (reproducible from same UUID)
 *   - Compatible with Memgraph string property constraints
 */
...
export class UnifiedId {
  ...
  public static tryParse(hex: string): UnifiedId | null {
    ...
    const firstIndex = UnifiedId.SYMBOLS.indexOf(hex[0] as (typeof UnifiedId.SYMBOLS)[number]);
    if (firstIndex < 0 || firstIndex >= 16) {
      return null;
    }

    for (const ch of hex) {
      if (!UnifiedId.SYMBOLS.includes(ch as (typeof UnifiedId.SYMBOLS)[number])) {
        return null;
      }
    }
  ...
  /** Returns the HEX32 string representation (13 chars, uppercase). */
  ...
  public equals(other: UnifiedId | bigint | string): boolean {
    ...
    if (typeof other === "string") {
      return this.toString().toUpperCase() === other.toUpperCase();
    }
    return false;
  }
  ...
  private static decode(hex: string): bigint {
    ...
    for (let grade = 0; grade < UnifiedId.LENGTH; grade++) {
      const index = BigInt(UnifiedId.SYMBOLS.indexOf(hex[grade] as (typeof UnifiedId.SYMBOLS)[number]));
      const shift = UnifiedId.X32_SHIFT * (UnifiedId.LENGTH - grade - 1);
